---
title: "Self-Organizing Map for contingency tables"
---
...
korresp.som <- trainSOM(x.data=presidentielles2002, dimension=c(8,8),
                        ...
                        radius.type="letremy", topo="square")
clusternames <- 1:prod(korresp.som$parameters$the.grid$dim)
korresp.som
```
...
by a hitmap:
```{r presiHitmap}
plot(korresp.som, what="obs", type="hitmap", print.title = F)
```
...
plot(korresp.som, what="obs", type="names", print.title = T)
```
...
plot(korresp.som, what="prototypes", type="lines", view="r", print.title=F) +    
  guides(color=guide_legend(keywidth=0.1, keyheight=0.1, default.unit="cm", ncol=2)) + 
  theme(legend.text=element_text(size=4), 
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
# plot the column prototypes (16 candidates)
plot(korresp.som, what="prototypes", type="lines", view="c", print.title=T) +
  guides(color=guide_legend(keywidth=0.1, keyheight=0.1,default.unit="cm", ncol=1)) + 
...
```{r presiGraphProto2}
plot(korresp.som, what="prototypes", type="poly.dist", print.title=F)
plot(korresp.som, what="prototypes", type="umatrix", print.title=F)
...
```{r presiSC3}
plot(my.sc, type="grid", print.title = T, the.titles=1:length(my.sc$cluster))
...
```{r presiSC4}
plot(my.sc, what="prototypes", type="hitmap", print.title = T, the.titles = clusternames)
plot(my.sc, what="prototypes", type="lines", print.title=F, view="c")
plot(my.sc, what="prototypes", type="poly.dist", print.title=T, the.titles = clusternames)
plot(my.sc, what="prototypes", type="mds")
```

And some others identify the super clusters with titles:
```{r presiSC5}
plot(my.sc, what="prototypes", type="color", view="r", variable="correze")
plot(my.sc, what="prototypes", type="color", view="c", variable="JOSPIN")
```
