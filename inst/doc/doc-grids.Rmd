...
%\VignetteIndexEntry{A basic vignettes on grids in SOMbrero}
-->

# Self-Organizing Maps with ```SOMbrero```
...
applied on myGrid class object:
...
* ```dimension```, which is vector of two integers. The first one is the x
...
choosen object. The only argument is the object to be printed.
...
the object and then call the function print previously described. The only 
...
The myGrid object plot function draws, in a new graphical window, the squared 
area corresponding to the object. It has 2 parameters:
...
this color. Yan can also give many colors: in this case, the number of colors 
must match the total number of squares. The map is filled from the bottom left
corner, up in the column and then the same in the next right column.
