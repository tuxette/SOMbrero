...
the shortest path between two characters (obtained with the
function ```shortest.paths``` of package ```igraph````). Note that its
...
```
On these graphics, one variable is represented respectively with a point, a 
bar or a slice. It is therefore easy to see which variable affects which 
cluster.
...
```
We can see that cluster 5 is very relevant to the story: as the
...
```
As we mentionned above, cluster 5 is far away from the others, it is 
...
```
* cluster 1 contains two of the main characters: ```Javert``` and ```Valjean```.
